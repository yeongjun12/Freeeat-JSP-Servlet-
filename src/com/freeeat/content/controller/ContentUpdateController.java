...
public class ContentUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		cont.setContNo(Integer.parseInt(request.getParameter("contNo")));
		...
		int result = new ContentService().updateContent(cont);
		
		if(result > 0) {
			request.getSession().setAttribute("successMsg", "콘텐츠를 수정했습니다.");
